...
extern uint8_t tx_msg_buf[VW_MAX_MESSAGE_LEN]={"\0"};
extern uint8_t tx_msg_buflen=VW_MAX_MESSAGE_LEN;
extern uint8_t tx_success=1;
...
inline uint8_t vw_have_message()
{
    return vw_rx_done;
}
...
void init1(const unsigned int periodHz,
  void (*userCallBack)())
{
  ...
  if(userCallBack != NULL)
    //userCB1 = userCallBack;
  aux_reg_write(timerLimitAddr, periodHz);  // Load Timer period

  aux_reg_write(timerCountAddr, 0);  // Reset variables
  tickCnt = 0;

 // if(isrFuncPtr1 != NULL) {  // Enable timer running with interrupt
    interrupt_connect(timerIrqNum, &timerIsr);
 ...
 uint8_t vw_get_decoded_msg(uint8_t* msg_buf, uint8_t* msg_buflen){


	uint8_t buf1[VW_MAX_MESSAGE_LEN];
	uint8_t buflen = VW_MAX_MESSAGE_LEN;
	uint8_t rx_sender_id=0;
	uint8_t rx_receiver_id=0;
	uint8_t rx_rpt=0;
	uint8_t ack=0;
	uint8_t seq=0;
	...
	if(vw_get_message(buf1, &buflen)){
		...
		rx_receiver_id=buf1[1];

		rx_rpt=buf1[2];
		ack=(rx_rpt&0x0F);
		rx_rpt=rx_rpt>>4;
		if (rx_receiver_id != SELF_ID && rx_receiver_id != 0x77){
			if (rx_rpt>0){
				rx_rpt-=1;
				rx_rpt=(rx_rpt<<4)|ack;
				buf1[2]=rx_rpt;
				vw_send(buf1,buflen);//see for true
			}
			return 0;
		}
		else{
			//memcpy(buf1+24,seq,1);
			seq=buf1[3];
			if (ack==1){
				tx_success=1;
				tx_msg_buf[0]=0x00;
				...
				memcpy(msg_buf,buf1+4,buflen-4);

				*msg_buflen=buflen-4;

			send_ack(rx_sender_id);
			return rx_sender_id;
			}
			else if (ack==3){
...
uint8_t tx_coded_msg(uint8_t* buf, uint8_t len, uint8_t rec_add, uint8_t ack){
	uint8_t sender_id=SELF_ID;
	uint8_t receiever_id=rec_add;
	uint8_t rpt=0x02;
	uint8_t seq_no=0x00;

	tx_msg_buf[0]=sender_id;
	tx_msg_buf[1]=receiever_id;
	rpt=(rpt<<4)|ack;
    tx_msg_buf[2]=rpt;
	tx_msg_buf[3]=seq_no;
	memcpy(tx_msg_buf+4,buf,len);
	tx_msg_buflen=len+4;
	tx_success=0;                           //TODO
	return(vw_send(tx_msg_buf,tx_msg_buflen));
}


void send_ack(uint8_t s_id){
	uint8_t tx_msg_buf[VW_MAX_MESSAGE_LEN];
	uint8_t sender_id=SELF_ID;
	uint8_t receiever_id=s_id;
	uint8_t rpt=0x01;
	uint8_t seq_no=0x00;
	tx_msg_buf[0]=sender_id;
	tx_msg_buf[1]=receiever_id;
	tx_msg_buf[2]=rpt;
	tx_msg_buf[3]=seq_no;
	vw_send(tx_msg_buf,4);
}

void tx_coded_msg_buff(){
	vw_send(tx_msg_buf,tx_msg_buflen);
	tx_success=0;
}
}
